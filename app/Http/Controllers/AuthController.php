...
class AuthController extends Controller
{
    ...
    {
        $this->temp_otp = $request['tempt_otp']; // Retrieve 'tempt_otp' from request
        Mail::to(Auth::user()->email)->send(new MailerLoginOtp(Auth::user()->email, $request['tempt_otp']));
        Session::put('temp_otp', $this->temp_otp);
        // return response()->json($request['tempt_otp']);
